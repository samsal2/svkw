...
public:
  ...
  constexpr explicit compressed_pair_leaf(U &&value) noexcept
...
template <size_t Idx, typename T> class compressed_pair_leaf<Idx, T, false> {
public:
  using value_type = T;
  using reference = value_type &;
  using const_reference = value_type const &;
  ...
  [[nodiscard]] constexpr value_type &get() noexcept { return value_; }
...
class compressed_pair : protected compressed_pair_leaf<0, T1>,
