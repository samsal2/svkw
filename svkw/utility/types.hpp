...
template <typename T>
[[nodiscard]] static constexpr std::byte *
force_cast_to_byte(T *data) noexcept {
...
struct is_compatible_with_element<
    ...
    : std::bool_constant<is_convertible_as_array_v<
          detail::value_type_from_data_t<Container>, Element>> {};
...
template <typename Iterator>
using is_random_access = std::is_same<get_iterator_category_t<Iterator>,
...
template <typename T> using uncvref_t = std::remove_cvref_t<T>;

template <typename T> static constexpr inline auto always_false = false;
