...
constexpr auto if_helper([[maybe_unused]] std::true_type condition,
                         Then is_true,
...
template <typename Then, typename Else>
constexpr auto if_helper([[maybe_unused]] std::false_type condition,
                         [[maybe_unused]] Then is_true,
                         Else is_false) noexcept {
  return is_false;
}

template <typename Tag> constexpr auto is_tag(Tag tag) noexcept {
...
constexpr auto find_if([[maybe_unused]] Condition condition,
...
template <typename This> struct matches_impl {
...
template <typename This> constexpr auto matches() {
