...
template <typename... Args> class any_wrapper;

template <typename Found> struct selected { using type = Found; };

template <typename Selected> using selected_t = typename Selected::type;

namespace options {
template <typename Deleter> struct deleter {};

template <typename Creator> struct creator {};
...
template <typename Storage> struct storage {};

template <auto Call> struct deleter_call {};

template <auto Call> struct creator_call {};
...
constexpr auto select([[maybe_unused]] Args... opt) {
  return utility::unpack_tag(
      utility::find_if(utility::tagged_with<Tag>(), utility::pack<Args...>{}));
}
